...
### 6.1. 🧠 Self-Learning Check (Auto — Ẩn khỏi User)

Sau khi code xong, AI TỰ ĐỘNG check và cập nhật skill files:

```
□ Có chọn tech/lib mới không? → Log vào adr-decisions/SKILL.md
□ Có fix bug nào không? → Check bug-fix-patterns/SKILL.md (TIL)
□ Bug fix lặp lại lần 2+? → Viết 5 Whys RCA vào bug-fix-patterns
□ Có optimize performance? → Log vào performance-playbook/SKILL.md
□ Task này sẽ lặp lại? → Log vào runbooks/SKILL.md
□ Search trước khi code? → Đã follow Tech Radar rule chưa?
```

**Quy tắc**: Grep skill file trước → nếu chưa có → append. KHÔNG lưu typo/lỗi 1 lần.
