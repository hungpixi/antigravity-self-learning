...
### 6.1. 🧠 Code Smell Catalog Update (Auto)

Sau khi refactor xong, AI TỰ ĐỘNG check:
```
□ Code smell vừa phát hiện đã có trong code-smell-catalog/SKILL.md chưa?
□ Nếu chưa → Append smell mới (format SMELL-xxx)
□ Refactor pattern hiệu quả? → Log vào prompt-patterns/SKILL.md
□ Performance cải thiện có metrics? → Log vào performance-playbook/SKILL.md
```
